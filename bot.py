...
async def go_guild(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("Отлично! Проверяю подписку...")
    if await is_subscribed(callback.from_user.id):
        ...
        await send_step(callback.message, "firstChoice",
            ...
            reply_markup=inline_kb([("🏋️ Войти в Храм (встретить Стёпу)", "enter_gym"), ("📱 Пойти налево (опасность)", "go_left")]))
        await state.set_state(GameState.first_choice)
    else:
        await callback.message.answer("Ты ещё не в гильдии. Подпишись и возвращайся.", reply_markup=inline_kb([("📢 Подписаться на канал", "go_guild")]))
...
async def accept_wisdom(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Ты покидаешь Стальной Храм, мышцы гудят после мудрых советов Арнольдыча. В животе заурчало – видимо, Фитосфера намекает, что пора подкрепиться. Ты отправляешься на поиски Кухни Изобилия. Говорят, там заправляет особа, которая даже брокколи превращает в искусство.",
...
async def process_pie(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "reject_pie":
        ...
        # Без изменений очков, но и без ошибок
...
async def process_question(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "strategy_change":
        ...
        # Переход к попугаю Кеше без гайда
        await callback.message.answer(
...
async def process_sleep(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "fight_lazy":
        ...
        await state.update_data(score=score, mistakes=mistakes)
        await callback.message.answer("Ты решил деградировать. Диван радостно урчит и обволакивает тебя пушистым пледом. Ты проспал свой прогресс, но, возможно, ещё не всё потеряно.",
...
async def process_enemy(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Пельменес облизнулся." if callback.data == "enemy_night" else "Печеньки атакуют." if callback.data == "enemy_sweet" else "Метаболизм страдает.")
    await callback.message.answer(
